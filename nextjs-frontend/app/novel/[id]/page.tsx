...
import { NovelDetail } from "@/lib/client/client.schemas";
...
export default async function NovelDetailPage({
  ...
  const [novel, error] = await tryAsync<NovelDetail, FetchError>(
    getNovelEndpointApiNovelsNovelIdGet((await params).id, {})
  );
  if (error?.status === 404) {
    return notFound();
  }

  novel.description = md.render(novel.description || "");
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-purple-950/10 to-background">
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <NovelBreadcrumb novelTitle={novel.title!} category="Isekai" />
        ...
        <NovelHero
          ...
          totalRatings={245}
          totalChapters={
            novel.volumes?.reduce(
              (acc, vol) => acc + (vol.chapters?.length || 0),
              0
            ) || 0
          }
        />
        ...
        <div className="space-y-8">
          ...
          <VolumesChapters volumes={novel.volumes!} />
